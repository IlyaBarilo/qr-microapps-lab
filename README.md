...
- содержит одиннадцать встроенных примеров: тест, «Как думает компьютер?», «Турнирная сетка», «Пакет в сети», «Разбей блоки», «Брандмауэр», «Успей в релиз», «Киберрефлекс», «Кибертрасса 3D», «Киберлабиринт 3D» и «Карьерный компас».

«Киберлабиринт 3D» использует собственный лучевой 3D-рендеринг без внешних библиотек: камера находится в точке персонажа, а каждый столбец стены рассчитывается по расстоянию от неё до препятствия. Карта 15×15 содержит четыре случайно выбираемые точки старта; кратчайший путь от каждой до выхода составляет 28 шагов. Из-за объёма 3D-логики пример использует коррекцию QR L.

Элементы каталога могут содержать необязательное поле `documentation` с введением и структурированными разделами (`paragraphs`, `items`, `diagram`, `visualization`). Тип визуализации `grid-map` строит компактную статичную SVG-карту из строк сетки, точек старта и выхода. Его контракт сохраняется для уже созданных документов: интерактивная карта или другой вид изображения должны получать новый `type` и отдельный рендерер. Кнопка «Описание примера» находится на этапе предпросмотра и активируется только для примеров с документацией; сейчас она добавлена для «Киберлабиринта 3D» и не включается в QR-нагрузку игры. Полный формат и правила обратной совместимости описаны в [спецификации создания игр](spec_game_creation_ru.md#12-документация-примера-в-каталоге).
