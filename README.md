...
Каталог `local/` и установленные каталоги `node_modules/` существуют только на компьютере разработчика и исключены из Git. В `local/archive/` сохранены ранние исследования, подробные методические документы и другие материалы, не нужные пользователю программы.
...
`AGENTS.md` и `.gitignore` являются локальными служебными файлами и намеренно не попадают в GitHub.
