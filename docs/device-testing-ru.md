...
Скриншоты нового испытания фиксируют запуск приложения из `data:` URL без подключения к Интернету и результат взаимодействия с ним. Перед запуском QR-код был повторно распознан штатным приложением «Фото» по сценарию, указанному в таблице.
