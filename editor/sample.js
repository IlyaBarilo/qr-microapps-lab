...
L=t=>{g=Math.min(2,(t-o)/16.7||1);o=t;if(!Q){X+=V*g;Y+=U*g;if(X<B||X>W-B)V=-V,X=X<B?B:W-B;if(Y<B)U=Math.abs(U),Y=B;if(U>0&&Y+B>=PY&&Y-U*g+B<=PY&&X>P&&X<P+J)Y=PY-B,U=-Math.abs(U);for(i=0;i<32;i++)if(A>>i&1){Z=6+i%8*29;T=35+(i>>3)*16;if(X+B>Z&&X-B<Z+25&&Y+B>T&&Y-B<T+14){A^=1<<i;K++;U=-U;if(!A)Q=3;break}}if(Y-B>H)Q=2}C[s]='#06131c';f(-O/S,0,w/S,H);for(i=0;i<32;i++)if(A>>i&1)C[s]='hsl('+i*43+',80%,60%)',f(6+i%8*29,35+(i>>3)*16,25,14);C[s]='#183b4b';f(0,PY+20,W,H-PY-20);C[s]='#5de4cf';f(P,PY,J,10);C.beginPath();C.arc(X,Y,B,0,7);C[s]='#fff';C.fill();C.font='bold 16px Arial';C.textAlign='left';C.fillText(K,12,24);if(Q){C[s]='#0c202ddd';f(22,120,196,110);C.textAlign='center';C.font='bold 20px Arial';C[s]='#fff';C.fillText(Q==3?'ПОБЕДА':Q==1?'РАЗБЕЙ БЛОКИ':'СЧЕТ '+K,120,168);C.font='13px Arial';C[s]='#9ab4c0';C.fillText(Q==3?'ГОТОВО':Q==1?'ВЕДИ ПАЛЬЦЕМ':'ЕЩЕ РАЗ',120,196)}requestAnimationFrame(L)};L()
  ...
  // Time is measured in 16 ms steps; the .25 correction preserves the original jump arc.
  var releaseRun = {
    id: 'release-run',
    title: 'Успей в релиз',
    html: `<!doctype html><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><canvas id=c style=position:fixed;inset:0;width:100%;height:100%;touch-action:none></canvas><script>\
var $d=3;x=c.getContext('2d');f=x.fillRect.bind(x);j=x.fillText.bind(x);F='fillStyle';n=Math.random;A=[];M=-20;m=1;q=s=l=T=0;onresize=z=_=>{W=innerWidth;H=innerHeight;Y=H-24;D=devicePixelRatio||1;c.width=W*D;c.height=H*D;x.scale(D,D);y=Y-22};z();\
N=_=>{A=[];m=q=s=l=0;u=2.9+$d*.3;v=0;y=Y-22;t=36};P=_=>m?N():q?l<1&&N():y>Y-23?v=-10.5:0;c.onpointerdown=e=>e.preventDefault(P());\
requestAnimationFrame(G=E=>{b=Math.min((E-T)/16,2);T=E;requestAnimationFrame(G);M=M>W+20?-20:M+.2*b;l>0&&(l-=b);if(!m&&!q){y+=(v+.25*b+.25)*b;v+=.5*b;y>Y-22&&(y=Y-22,v=0);if((t-=b)<1){k=n()*3|0;h=[16,30,22][k];w=(k?18:22)*(n()*3+1|0);A.push([W+w,Y-h,w,h,k,0]);t=58-$d*4+n()*(32-$d*2);u+=.004+$d*.002}for(i=A.length;i--;){a=A[i];a[0]-=u*b;r=a[0]+a[2];if(!a[5]&&r<44)s++,a[5]=1;48<r&&58>a[0]&&y+20>a[1]&&(q=1,l=36);a[0]<-a[2]&&A.splice(i,1)}}\
x[F]='#124';f(0,0,W,H);x[F]='#789';for(i=18;i--;)f(i*79%W,i*53%(H*.45),2,2);x[F]='#fd7';x.beginPath();x.arc(M,86,11,0,7);x.fill();x[F]='#124';x.beginPath();x.arc(M+4,83,8,0,7);x.fill();x[F]='#333';f(0,Y,W,24);x[F]='#7cf';f(0,Y-4,W,4);x[F]='#fff';x.font='bold 18px sans-serif';x.textAlign='left';j(s,32,64);for(i=A.length;i--;){a=A[i];x[F]=a[4]==1?'#f80':a[4]==2?'#bbb':'#f45';f(a[0],a[1],a[2],a[3])}x[F]='#8ef';f(44,y,18,22);\
if(m||q){B=H/2;C=W/2;x[F]='#123e';f(C-86,B-44,172,88);x[F]='#7cf';x.textAlign='center';j('УСПЕЙ В РЕЛИЗ',C,B-14);x.font='15px sans-serif';x[F]='#fff';j(q?'СЧЕТ '+s:'ТАП-ПРЫЖОК',C,B+10);q&&l<1&&j('ЕЩЕ',C,B+32);m&&j('СТАРТ',C,B+32)}})</script>`,
    spec: {
  ...
  var cyberReflex = {
    id: 'cyber-reflex',
    title: 'Киберрефлекс',
    html: `<!doctype html><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><style>body{margin:0;color:#fff;font:700 18px system-ui;text-align:center;touch-action:none;user-select:none}main{height:100vh;display:grid;place-content:center;background:radial-gradient(#124,#001)}h1{font-size:10vw;margin:8px}#o{position:relative;width:min(50vw,25vh);aspect-ratio:1;margin:auto;background:#124}#o,button{border-radius:50%}.w button{background:#012}.f button{background:#af2;color:#012}.f button:after{content:'ЖМИ!'}#t{font-size:8vw;color:#2ef}button{position:absolute;inset:6px;border:0;background:#2ef;color:#fff;font:700 min(9vw,6vh) system-ui}</style><main id=m><b id=h>5 РАУНДОВ</b><h1>КИБЕР<br>РЕФЛЕКС</h1><div id=o><button id=b>СТАРТ</button></div><div id=t></div></main><script>var $d=3,r=0,s=0,a=[],q,T,p=performance;m.onpointerdown=e=>s?x(s<2?-1:p.now()-T):e.target==b&&!r&&(a=[],o.style='',b.innerText='',n());function n(){s=1;m.className='w';h.innerText=++r+'/5 РАУНД';t.innerText='ЖДИ';q=setTimeout(()=>{s=2;m.className='f';T=p.now();q=setInterval(()=>{t.innerText=z=p.now()-T|0;z>1e3-$d*100&&x(-1)},16)},1e3+Math.random()*2e3)}function x(v){clearTimeout(q);s=0;m.className='w';z=v<0;v=z?1e3-$d*100:v|0;a.push(v);o.style.background=r<5?'conic-gradient(#2ef '+20*r+'%,#124 0)':'#2ef';t.innerHTML=z?'<font color=#f90>ШТРАФ</font>':v+' мс';setTimeout(r<5?n:y,z?999:1500)}function y(){let v=a.reduce((x,y)=>x+y)/5|0;T=Math.min(...a);r=0;m.className='';h.innerText='СРЕДНИЙ '+v+' · МИН. '+T+' мс';t.innerHTML=(v<250?'МОЛНИЯ':v<301?'ПРО':v<500?'ИГРОК':'РАЗМИНКА')+'<br>'+v+' мс';b.innerText='СНОВА'}</script>`,
    spec: {
